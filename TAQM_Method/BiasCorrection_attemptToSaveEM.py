#!/usr/bin/env python
# coding: utf-8
### Script for doing bias correction, where we use raw and calibrated quantiles to also estimate ensemble members. Target year, which-initialisation (1 to 4), and target month will be system/cmndline argument, so we can run this using sbatch.
...
#Let's try to save a full-field
rawEMSIC=np.empty((30,Grdlen))
calEMSIC=np.empty((30,Grdlen))
...
filename = save_path+'Forecast_Calibration_incl_EMs_Yr'+str(targetyear)+'_'+str(whichinit).zfill(2)+'Mn_'+str(leadtimeMonth).zfill(2)+'.nc'
...
Raw_SIC_byEM_towrite=ncfile.createVariable('SIC_RAW_EM', 'd',  ('ens','n2d'))
Cal_SIC_byEM_towrite=ncfile.createVariable('SIC_Cal_EM', 'd',  ('ens','n2d'))
...
Raw_SIC_byEM_towrite[:]=rawEMSIC
Cal_SIC_byEM_towrite[:]=calEMSIC
