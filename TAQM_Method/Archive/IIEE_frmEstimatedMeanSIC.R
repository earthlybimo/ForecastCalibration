### Script to open calibrated SSIPS forecast, compute IIEE from estimated Mean, then save it into a table.

library(ncdf4);library(spheRlab)

save_path = '/work/ba1138/a270138/BiasCorrOutput/TAQMResults/MeanConc'
Allsavename=paste0("/work/ba1138/a270138/BiasCorrOutput/EstMeanConc_IIEEResults")
...
for(yy in 1:length(inYR)){
  for(init in 1:4){
    for(mm in 1:12){
      ...
      remove(O,U,temp,rawSIP,tempU,tempO)
      
      calSIP=binarise(calSIC,0.15)
      temp=(obsSIP-calSIP)
      tempU=array(0,dim=dim(temp));tempO=tempU
      ...
      remove(O,U,temp,calSIP,tempU,tempO)
      
      remove(rawSIC,calSIC,obsSIP)
