### Script to open calibrated SSIPS forecast FROM LongCalibrated set, compute SPS, then save it into a table.

library(ncdf4);library(spheRlab)

save_path = '/work/ba1138/a270138/BiasCorrOutput/TAQMResults'
Allsavename=paste0(save_path,"/CollectedLongCalSPSResult")
...
for(yy in 1:length(inYR)){
  for(init in 1:4){
    for(mm in 1:12){
      loadname=sprintf("%s/Forecast_Calibration_fromLongCalib_Yr%d_%02dMn_%02d.nc",save_path,inYR[yy],init,mm)
