...
for( y1 in 1:length(histYrs)){
  ...
  for(EM in 1:30){
    ...
    nc_close(fl)
    histFcst[EM,,]=sic0
    sip0=binarise(sic0,0.15)
    histbias[EM,,]=sip0-histObs
  }
...
for(EM in 1:30){
  ...
  sip0=binarise(sic0,0.15)
  rawFcst[EM,,]=sip0
  bcalFcst[EM,,]=sip0-meanHistbias
}

bcalFcst[bcalFcst>1]=1
bcalFcst[bcalFcst<0]=0

## To find SPS, average 'cal fcst' over EM to get Probability
bcalSIP=apply(bcalFcst,c(2,3),mean,na.rm=T)
rawSIP=apply(rawFcst,c(2,3),mean,na.rm=T)
...
save(file = savename,version = 2,SPSrawSH,SPSrawNH,SPSbcalSH,SPSbcalNH,IIEErawSH,IIEEbcalSH,IIEErawNH,IIEEbcalNH,bcalFcst,meanHistbias)
print(paste0("Done! Bias removed and saved SIP (and SPS/IIEE) here: ",basename(savename)))
